...
## 2026-09-19：4、6、7 号机未就绪修复

现场读取三架 `ready=false`，原因均为定位桥未就绪、点云无更新；桥状态为 `waiting_for_lio`，MID360 原始点云和 IMU 都无消息。eno1 只有飞控及 SSH 网段地址，缺少 MID360 JSON 中配置的主机地址。

仅修复 4、6、7：运行时恢复 eno1 的 192.168.1.204/24、192.168.1.206/24、192.168.1.207/24，并追加到 NetworkManager「有线连接 1」的 ipv4.addresses；保留原飞控与 SSH 地址。回读持久配置确认成功，尚未通过整机重启验证。

确认 fresh、已着陆、未解锁、无执行任务后，通过现有停止/启动流程重新启动三架容器并上传当前代理，驱动重新绑定网络地址。最终三架均 ready=true、reasons=[]、bridge_ready=true；点云分别为 746、803、668 点。均 IDLE、未解锁、已着陆且无控制器。未发送飞行、解锁或导航目标；未修改其他飞机。
