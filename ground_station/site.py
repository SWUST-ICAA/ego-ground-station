...
def build_plan(profile,targets,state):
    ...
    if 'flight-v2' not in state.get('capabilities',[]):raise ValueError('机载代理需更新坐标与跳点功能；请先关闭程序再重新启动')
    ...
    for p in targets:
        if p['kind']=='local' and p.get('frame')!='right-forward':
            raise ValueError('旧米制航点使用 map 坐标，请按 X 向右、Y 向前重新保存航点')
        goal=to_local(p['a'],p['b'],anchor) if p['kind']=='geo' else to_map([p['a'],p['b']],frame)
    ...
    return dict(waypoints=[dict(kind='local',a=p[0],b=p[1]) for p in points],
                flight_plan=dict(polygon=polygon,margin=fence.margin,mode=profile['mode'],
                                 origin=start,return_points=returns,geo_anchor=anchor if uses_geo else None,local_frame=frame),
